class QuestItemsController < ApplicationController
  ...
  def update
    respond_to do |format|
      if @quest_item.update(quest_item_params)
        format.html { redirect_to @quest_item, notice: "Quest item was successfully updated.", status: :see_other }
  ...
  def destroy
    ...
    respond_to do |format|
      format.html { redirect_to quest_items_path, notice: "Quest item was successfully destroyed.", status: :see_other }
  ...
  private
    ...
    def quest_item_params
      params.expect(quest_item: [ :quest, :is_done ])
    end
end
